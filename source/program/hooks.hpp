...
static constexpr int arraySize = 258;
static char armorSortInfo[arraySize * 0x1c0] = {0};
static ArmorSortInfo *armorSortInfoPtrs[arraySize] = {0};
...
HOOK_DEFINE_TRAMPOLINE(sortUpdateGameData){
	static void Callback(void *param_1, int param_2, uint param_3, long param_4){
		char buf[500];
...
HOOK_DEFINE_INLINE(SetCalculatedSizeX9){
	static void Callback(exl::hook::InlineCtx * ctx){
		char buf[500];
uint64_t calculated = ((arraySize - 1) * 0x1c0) + 0x20 + 0x8b8;
...
// Hook 6: 0x7101a7324c || 0x7101a73240 || 0x7101a73238
